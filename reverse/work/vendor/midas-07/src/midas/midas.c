...
RCSID(const char *midas_rcsid = "$Id: midas.c,v 1.14 1997/03/05 19:35:03 pekangas Exp $";)
...
/* Disable all virtual memory support from DJGPP: */
#ifdef __DJGPP__
#include <crt0.h>
...
void CALLING midasSetDefaults(void)
{
    ...
    mDSoundMode = M_DSOUND_DISABLED;    /* DirectSound disabled */
    mDSoundObject = NULL;               /* DirectSound object not set */
    mDSoundHwnd = NULL;                 /* DirectSound HWND not set */
    mDSoundBufferLength = 100;          /* 100ms buffer default */

    m16bitUlawAutoConvert = 1;          /* convert 16-bit samples to u-law */
}
