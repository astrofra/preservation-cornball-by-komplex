 ...
 *
 * $Id: dsound.c,v 1.7 1997/02/19 21:43:45 pekangas Exp $
 *
...
RCSID(char const *dsound_rcsid = "$Id: dsound.c,v 1.7 1997/02/19 21:43:45 pekangas Exp $";)

#define DSVERSION 1.00
#define DSVERSTR "1.00"
...
/* Local prototypes: */
int CALLING dsSetAmplification(unsigned _amplification);
int CALLING dsGetAmplification(unsigned *_amplification);
...
int CALLING dsDetect(int *result)
{
    ...
    if ( (dsResult = DirectSoundCreate(NULL, &ds, NULL)) != DS_OK )
    {
...
int CALLING dsInit(unsigned _mixRate, unsigned mode)
{
    ...
    switch ( outputMode )
    {
        ...
        default:
            LeaveCriticalSection(&deviceUsed);
            ERROR(errInvalidArguments, ID_dsInit);
...
/****************************************************************************\
*
* Function:     int dsOpenChannels(unsigned channels)
*
...
int CALLING dsOpenChannels(unsigned channels)
{
    int         error;

    /* Open DSM channels: */
    if ( (error = dsmOpenChannels(channels)) != OK )
        PASSERROR(ID_dsOpenChannels)
...
*/
        if ( channels < 5 )
            dsSetAmplification(64);
        else
            dsSetAmplification(14*channels);
/*
    }
*/
...
/****************************************************************************\
*
* Function:     int dsSetAmplification(unsigned amplification)
*
...
int CALLING dsSetAmplification(unsigned _amplification)
{
    ...
    else
    {
        ...
        if ( (error = dsmSetAmplification(amplification)) != OK )
            PASSERROR(ID_dsSetAmplification)
    }
...
/****************************************************************************\
*
* Function:     int dsGetAmplification(unsigned *amplification);
*
...
int CALLING dsPlay(int *callMP)
{
    ...
    while ( spaceLeft && mixLeft )
    {
        ...
        /* Check that we won't mix more data than what fits to DSM mixing
           buffer: */
        ...
        if ( l1 )
            postProc(l1 / mixElemSize, p1, 0, dsmMixBuffer, ppTable);
        if ( l2 )
        {
            if ( outputMode & sdStereo )
                postProc(l2 / mixElemSize, p2, 0,
                    dsmMixBuffer + (l1/2), ppTable);
            else
                postProc(l2 / mixElemSize, p2, 0,
                    dsmMixBuffer + (l1/2), ppTable);
        }
