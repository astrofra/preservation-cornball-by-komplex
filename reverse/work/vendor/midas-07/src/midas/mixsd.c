 ...
 *
 * $Id: mixsd.c,v 1.3 1997/02/27 16:04:37 pekangas Exp $
 *
...
RCSID(const char *mixsd_rcsid = "$Id: mixsd.c,v 1.3 1997/02/27 16:04:37 pekangas Exp $";)
...
int CALLING mixsdPlay(int *callMP)
{
    ...
    int         dsmBufSize;
    int         dmaBufLeft;
#if defined(DUMPBUFFER) || defined(__DJGPP__)
    unsigned    oldPos;
#endif
    ...
    while ( numElems )
    {
        ...
        if ( dmaBufLeft < mixNow )
        {
            ...
            oldPos = mixPos;
#endif
            postProc(dmaBufLeft, bufferPtr, mixPos, mixBuf,
                ppTable);
#ifdef __DJGPP__
            dosmemput(bufferPtr + oldPos, dmaElemSize * dmaBufLeft,
                buffer.startAddr + oldPos);
#endif
#ifdef DUMPBUFFER
        ...
        oldPos = mixPos;
#endif
        mixPos = postProc(mixNow, bufferPtr, mixPos, mixBuf,
            ppTable);
#ifdef __DJGPP__
        dosmemput(bufferPtr + oldPos, dmaElemSize * mixNow,
            buffer.startAddr + oldPos);
#endif
#ifdef DUMPBUFFER
