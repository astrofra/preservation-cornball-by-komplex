...
#*
#* $Id: wcopts.mak,v 1.7 1997/02/27 16:22:44 pekangas Exp $
#*
...
ifdef _WIN32
  BUILDLIBRARY = wlib -c -n $@ $(patsubst %,+-%,$^)
  define BUILDDLLIMPLIB
	$(LINK) $(DLLLINKOPTS) name $@ $(patsubst %,file %, $^)
	wlib -c $(IMPLIBNAME) +-$@
  endef
endif
...
ifdef _DOS
  ...
  define BUILDLIBRARY
	...
	@$(REM) $(foreach _I,$^, $(shell echo +-$(_I) >> libtemp.tmp)))
	wlib -c -n $@ @libtemp.tmp
	$(RM) libtemp.tmp
  endef
endif
...
BUILDEXE = $(LINK) $(LINKOPTS) name $@ $(patsubst %, file %, $^)
