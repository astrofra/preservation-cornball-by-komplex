...
static int asset_root_looks_valid(const char *root)
{
    ...
    join_path(surf128_path, sizeof(surf128_path), root, "SURF128.TGA");
    join_path(fla_path, sizeof(fla_path), root, "FLA.TGA");
    join_path(txt1_path, sizeof(txt1_path), root, "TXT1.TGA");
    return file_exists(surf128_path) && file_exists(fla_path) && file_exists(txt1_path);
}
...
static void warmup_scene(PreviewApp *app)
{
    ...
    cornball_surf_scene_step_frame(&app->scene, &app->random, scene_time, &app->frame);
...
static LRESULT CALLBACK preview_wndproc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    case WM_SIZE:
        if (app != NULL) {
            ...
            if (app->hglrc != NULL) {
                cornball_surf_gl_resize(&app->renderer, app->width, app->height);
            }
        }
        return 0;
...
{
    const char *class_name = "CornballSurfPreviewWindow";
    WNDCLASSA wc;
    RECT rect;
...
static int update_and_render(PreviewApp *app)
{
    ...
    cornball_surf_gl_render(&app->renderer, &app->frame);
    if (!capture_current_frame(app, "surf")) {
        return 0;
    }
    SwapBuffers(app->hdc);
...
int main(int argc, char **argv)
{
    ...
    if (!resolve_asset_root(&app.options)) {
        fprintf(stderr, "Could not resolve a valid asset root containing SURF128.TGA, FLA.TGA, and TXT1.TGA.\n");
        return 1;
    }

    cornball_random_seed(&app.random, app.options.random_seed);
    cornball_surf_scene_clear(&app.scene);
    cornball_surf_scene_loader_pass(&app.scene);
    warmup_scene(&app);
    ...
    if (!cornball_surf_gl_init(&app.renderer, app.options.asset_root, error_message, sizeof(error_message))) {
    ...
    cornball_surf_gl_shutdown(&app.renderer);
    destroy_gl_window(&app);
    return 0;
}
